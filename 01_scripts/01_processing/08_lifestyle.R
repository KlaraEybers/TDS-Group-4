...
summary(ukb_clean$sleep_hrs.0.0)

# Extract relevant# Extract relevant# Extract relevant columns ------------------------------------------------
...
lifestyle_clean <- selected_data %>%
  mutate(
    # ----------------------------------------------------------
    # Sleep duration (numeric + Short/Normal/Long), (-1/-3 -> NA)
    # ----------------------------------------------------------
    ...
    # ----------------------------------------------------------
    # Sleep Categories (Short/Normal/Long sleep)
    # ----------------------------------------------------------
    sleep_cat = case_when(
      sleep_hrs <= 6 ~ "Short",
    ...
    # ----------------------------------------------------------
    # Smoking but refined version
    # ----------------------------------------------------------
    # Clean age stopped smoking into numeric, (-1/-3 -> NA)
    ...
    # ----------------------------------------------------------
    # Refined smoking status - (DNK/PNA -> NA)
    # ----------------------------------------------------------
    smoking_status_refined = case_when(
      ...
      # Former smokers with missing quit time -> NA
    ...
    # ----------------------------------------------------------
    # Pack years - Cap implausible values
    # ----------------------------------------------------------
    ...
    # ----------------------------------------------------------
    # Passive smoking hours (home/outside) - (-1/-3 -> NA), Cap implausible values
    # ----------------------------------------------------------
    ...
    # Flag if total was computed from partial information (For myself to look at)
    passive_partial_flag = ifelse(
      xor(is.na(tobacco_home_hrs), is.na(tobacco_out_hrs)),
      1, 0
    ),
    
    # >168 hours/week is impossible -> NA
    passive_total_hrs = ifelse(
  ...
  mutate(
    # ----------------------------------------------------------
    # Passive smoking categories (None/Low/High)
    # ----------------------------------------------------------
