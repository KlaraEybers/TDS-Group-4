...
This project uses separate environments for the R and Python components:
 
1. Create the R conda environment:
 
   conda env create -f environment_r413.yml
...
4. Install Python packages:
   ...
   eval "$(~/anaconda3/bin/conda shell.bash hook)"
   conda activate group4_python_environment
...
order, adapting the working directory in each job script to match yours. 

~/TDS_Group4/03_jobs/
  1-pre-processing.sh
  2-imputation.sh
  3-exploratory.sh
  4-stability-selection.sh
  5-refit.sh
  6-ml.sh
  7-sensitivity-selection.sh
  8-sensitivity-analysis.sh
...
Step 4 — Variable selection:
   ...
   qsub 03_jobs/04-stability_selection.sh
 
   LASSO stability selection using the sharp package on the selection set (50%).
   Separate runs for outcome model and mediator model.
   Output: 00_data/02_analysis/
 
Step 5 — Model refitting:
 
   qsub 03_jobs/05-refit.sh
 
   Mediation analysis (total/direct/indirect effects with bootstrapped CIs) and
   prediction model evaluation on the held-out test set.
   Output: 02_results/
 
Step 6 — Machine learning models:
 
   qsub 03_jobs/06-ml.sh
   
   Uses the environment contained in TDS_Group4: "group4_python_environment". 
   Adapt the path to your environment if there are issues. Python requirements
   are in "python_requirements.txt"
...
Step 7 — Sensitivity analyses:
   ...
   qsub 03_jobs/07-sensitivity.sh
 
   Sex-stratified analysis, complete case analysis, subgroup performance evaluation.
   Output: 02_results/
